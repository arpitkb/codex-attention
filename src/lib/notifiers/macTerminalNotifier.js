...
export function buildTerminalNotifierArgs({ title, message, config, sessionId, eventName }) {
  const args = [
    '-title',
    title,
    '-message',
...
export async function sendTerminalNotification({ title, message, config, sessionId, eventName, env = process.env }) {
  const command = findTerminalNotifier(env)
  if (!command) {
    return { ok: false, error: new Error('terminal-notifier not found') }
  }

  const args = buildTerminalNotifierArgs({ title, message, config, sessionId, eventName })
